...
export default {
    data () {
        return {
            list: ['egg', 'rice', 'bread', 'milk', 'noodle', 'meat'],
            voiceList: [],
            imgList: [],
            curIndex: -1,
            curSrc: ''
    ...
    methods: {
        ...
        initVoices () {
            this.list.map(item => {
                this.voiceList.push(import(/* webpackPrefetch: true */ 'assets/english/food/' + (item) + '.mp3'));
                this.imgList.push(require('assets/english/food/' + (item) + '.png'));
            })
        },
