...
export default {
    data () {
        return {
            list: ['bike', 'motorcycle', 'car', 'bus', 'ship', 'train', 'High-speed rail', 'plane'],
            voiceList: [],
            imgList: [],
            curIndex: -1,
            curSrc: ''
    ...
    methods: {
        ...
        initVoices () {
            this.list.map(item => {
                this.voiceList.push(import(/* webpackPrefetch: true */ 'assets/english/vehicle/' + (item) + '.mp3'));
                this.imgList.push(require('assets/english/vehicle/' + (item) + '.png'));
            })
        },
