...
export default {
    data () {
        return {
            list: ['head', 'hair', 'ear', 'eye', 'nose', 'mouth', 'face', 'arm', 'hand', 'leg', 'foot'],
            voiceList: [],
            imgList: [],
            curIndex: -1,
            curSrc: ''
    ...
    methods: {
        ...
        initVoices () {
            this.list.map(item => {
                this.voiceList.push(import(/* webpackPrefetch: true */ 'assets/english/body/' + (item) + '.mp3'));
                this.imgList.push(require('assets/english/body/' + (item) + '.png'));
            })
        },
